...
import { getEntities, reset } from './cooperative.reducer';
...
export const Cooperative = () => {
  ...
  const totalItems = useAppSelector(state => state.cooperative.totalItems);
  const links = useAppSelector(state => state.cooperative.links);
  const entity = useAppSelector(state => state.cooperative.entity);
  ...
  return (
    <div>
      ...
      <div className="table-responsive">
        <InfiniteScroll
          dataLength={cooperativeList ? cooperativeList.length : 0}
          next={handleLoadMore}
              ...
              <thead>
                <tr>
                  <th className="hand" onClick={sort('id')}>
                    <Translate contentKey="coopcycleApp.cooperative.id">ID</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('name')}>
                    <Translate contentKey="coopcycleApp.cooperative.name">Name</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('zone')}>
                    <Translate contentKey="coopcycleApp.cooperative.zone">Zone</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
                    <Translate contentKey="coopcycleApp.cooperative.idDG">Id DG</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {cooperativeList.map((cooperative, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{cooperative.zone}</td>
                    <td>{cooperative.idDG ? <Link to={`/utilisateur/${cooperative.idDG.id}`}>{cooperative.idDG.id}</Link> : ''}</td>
                    <td className="text-end">
                      <div className="btn-group flex-btn-group-container">
                        ...
                        <Button
                          tag={Link}
                          to={`/cooperative/${cooperative.id}/delete`}
                          color="danger"
                          size="sm"
