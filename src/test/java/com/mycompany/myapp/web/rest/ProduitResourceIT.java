...
class ProduitResourceIT {
    ...
    public static Produit createEntity(EntityManager em) {
        Produit produit = new Produit().name(DEFAULT_NAME).prix(DEFAULT_PRIX);
        // Add required entity
        Boutique boutique;
        if (TestUtil.findAll(em, Boutique.class).isEmpty()) {
            boutique = BoutiqueResourceIT.createEntity(em);
            em.persist(boutique);
            em.flush();
        } else {
    ...
    public static Produit createUpdatedEntity(EntityManager em) {
        Produit produit = new Produit().name(UPDATED_NAME).prix(UPDATED_PRIX);
        // Add required entity
        Boutique boutique;
        if (TestUtil.findAll(em, Boutique.class).isEmpty()) {
            boutique = BoutiqueResourceIT.createUpdatedEntity(em);
            em.persist(boutique);
            em.flush();
        } else {
            boutique = TestUtil.findAll(em, Boutique.class).get(0);
    ...
    void putExistingProduit() throws Exception {
        ...
        updatedProduit.name(UPDATED_NAME).prix(UPDATED_PRIX);
    ...
    void fullUpdateProduitWithPatch() throws Exception {
        ...
        partialUpdatedProduit.name(UPDATED_NAME).prix(UPDATED_PRIX);
