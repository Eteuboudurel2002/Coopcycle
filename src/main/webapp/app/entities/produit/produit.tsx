...
export const Produit = () => {
  ...
  const entity = useAppSelector(state => state.produit.entity);
  ...
  return (
    <div>
      <h2 id="produit-heading" data-cy="ProduitHeading">
        <Translate contentKey="coopcycleApp.produit.home.title">Produits</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="coopcycleApp.produit.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/produit/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="coopcycleApp.produit.home.createLabel">Create new Produit</Translate>
          </Link>
        </div>
      </h2>
          ...
          {produitList && produitList.length > 0 ? (
              ...
              <thead>
                <tr>
                  <th className="hand" onClick={sort('id')}>
                    <Translate contentKey="coopcycleApp.produit.id">ID</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('name')}>
                    <Translate contentKey="coopcycleApp.produit.name">Name</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('prix')}>
                    <Translate contentKey="coopcycleApp.produit.prix">Prix</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
                    <Translate contentKey="coopcycleApp.produit.idboutique">Idboutique</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {produitList.map((produit, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{produit.name}</td>
                    <td>{produit.prix}</td>
                    <td>{produit.idboutique ? <Link to={`/boutique/${produit.idboutique.id}`}>{produit.idboutique.id}</Link> : ''}</td>
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="coopcycleApp.produit.home.notFound">No Produits found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
